...
import (
	...
	"github.com/Eliatra/opensearch-k8s-operator/opensearch-operator/pkg/reconcilers"
...
//+kubebuilder:rbac:groups="opensearch.eliatra.io",resources=events,verbs=create;patch
//+kubebuilder:rbac:groups=opensearch.eliatra.io,resources=opensearchclusters,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=opensearch.eliatra.io,resources=opensearchclusters/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=opensearch.eliatra.io,resources=opensearchclusters/finalizers,verbs=update
...
func (r *OpenSearchClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	myFinalizerName := "Eliatra"

	r.Instance = &eliatrav1.OpenSearchCluster{}
	...
	if r.Instance.Status.Phase == "" {
		r.Instance.Status.Phase = eliatrav1.PhasePending
	}

	switch r.Instance.Status.Phase {
	case eliatrav1.PhasePending:
		return r.reconcilePhasePending(ctx)
	case eliatrav1.PhaseRunning:
...
func (r *OpenSearchClusterReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&eliatrav1.OpenSearchCluster{}).
		Owns(&corev1.Pod{}).
		Owns(&corev1.Secret{}).
...
func (r *OpenSearchClusterReconciler) reconcilePhasePending(ctx context.Context) (ctrl.Result, error) {
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		r.Instance.Status.Phase = eliatrav1.PhaseRunning
		r.Instance.Status.ComponentsStatus = make([]eliatrav1.ComponentStatus, 0)
