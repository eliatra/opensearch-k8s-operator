...
func ClusterDescWithVersion(version string) eliatrav1.OpenSearchCluster {
...
func ClusterDescWithKeystoreSecret(secretName string, keyMappings map[string]string) eliatrav1.OpenSearchCluster {
	return eliatrav1.OpenSearchCluster{
		Spec: eliatrav1.ClusterSpec{
			General: eliatrav1.GeneralConfig{
				Keystore: []eliatrav1.KeystoreValue{
					{
...
func ClusterDescWithAdditionalConfigs(addtitionalConfig map[string]string, bootstrapAdditionalConfig map[string]string) eliatrav1.OpenSearchCluster {
	return eliatrav1.OpenSearchCluster{
		Spec: eliatrav1.ClusterSpec{
			General: eliatrav1.GeneralConfig{
				AdditionalConfig: addtitionalConfig,
			},
			Bootstrap: eliatrav1.BootstrapConfig{
...
var _ = Describe("Builders", func() {
	When("Constructing a STS for a NodePool", func() {
		It("should include the init containers as SKIP_INIT_CONTAINER is not set", func() {
			...
			result := NewSTSForNodePool("foobar", &clusterObject, eliatrav1.NodePool{}, "foobar", nil, nil, nil)
		...
		It("should use General.DefaultRepo for the InitHelper image if configured", func() {
			...
			result := NewSTSForNodePool("foobar", &clusterObject, eliatrav1.NodePool{}, "foobar", nil, nil, nil)
			Expect(result.Spec.Template.Spec.InitContainers[0].Image).To(Equal("mycustomrepo.cr/eoko-busybox:1.27.2-buildx"))
		})
		It("should use InitHelper.Image as InitHelper image if configured", func() {
			clusterObject := ClusterDescWithVersion("2.2.1")
			customImage := "mycustomrepo.cr/custombusybox:1.2.3"
			clusterObject.Spec.InitHelper = eliatrav1.InitHelperConfig{
				ImageSpec: &eliatrav1.ImageSpec{
					Image: &customImage,
				},
			}
			result := NewSTSForNodePool("foobar", &clusterObject, eliatrav1.NodePool{}, "foobar", nil, nil, nil)
		...
		It("should use defaults when no custom image is configured for InitHelper image", func() {
			clusterObject := ClusterDescWithVersion("2.2.1")
			result := NewSTSForNodePool("foobar", &clusterObject, eliatrav1.NodePool{}, "foobar", nil, nil, nil)
			Expect(result.Spec.Template.Spec.InitContainers[0].Image).To(Equal("docker.io/eliatra/eoko-busybox:1.27.2-buildx"))
		})
		...
		It("should use default storageclass if not specified", func() {
			...
			nodePool := eliatrav1.NodePool{
				...
				Component: "masters",
				Roles:     []string{"cluster_manager", "data"},
				Persistence: &eliatrav1.PersistenceConfig{PersistenceSource: eliatrav1.PersistenceSource{
					PVC: &eliatrav1.PVCSource{
	...
	When("When Reconciling the snapshotRepoJob", func() {
		It("should create a snapshotconfig batch job", func() {
			...
			snapshotConfig := eliatrav1.SnapshotRepoConfig{
				Name:     "os-snap",
				Type:     "s3",
				Settings: snapshotRepoSettings,
			}
			clusterObject.Spec.General.SnapshotRepositories = []eliatrav1.SnapshotRepoConfig{snapshotConfig}
	...
	When("Constructing a bootstrap pod", func() {
		It("should use General.DefaultRepo for the InitHelper image if configured", func() {
			...
			clusterObject.Spec.General.DefaultRepo = &customRepository
			result := NewBootstrapPod(&clusterObject, nil, nil)
			Expect(result.Spec.InitContainers[0].Image).To(Equal("mycustomrepo.cr/eoko-busybox:1.27.2-buildx"))
		})
