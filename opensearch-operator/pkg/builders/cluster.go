...
const (
	ConfigurationChecksumAnnotation      = "eliatra.io/config"
...
func NewHeadlessServiceForNodePool(cr *eliatrav1.OpenSearchCluster, nodePool *eliatrav1.NodePool) *corev1.Service {
	labels := map[string]string{
		helpers.ClusterLabel:  cr.Name,
...
func NewServiceForCR(cr *eliatrav1.OpenSearchCluster) *corev1.Service {
	labels := map[string]string{
		helpers.ClusterLabel: cr.Name,
	}
	return &corev1.Service{
...
func NewDiscoveryServiceForCR(cr *eliatrav1.OpenSearchCluster) *corev1.Service {
	labels := map[string]string{
		helpers.ClusterLabel: cr.Name,
	}
...
func NewNodePortService(cr *eliatrav1.OpenSearchCluster) *corev1.Service {
	labels := map[string]string{
		helpers.ClusterLabel: cr.Name,
	}
...
func PortForCluster(cr *eliatrav1.OpenSearchCluster) int32 {
	httpPort := int32(9200)
...
func URLForCluster(cr *eliatrav1.OpenSearchCluster) string {
	httpPort := PortForCluster(cr)
	return fmt.Sprintf("https://%s.svc.%s:%d", DnsOfService(cr), helpers.ClusterDnsBase(), httpPort)
}

func PasswordSecret(cr *eliatrav1.OpenSearchCluster, username, password string) *corev1.Secret {
...
func DnsOfService(cr *eliatrav1.OpenSearchCluster) string {
	return fmt.Sprintf("%s.%s", cr.Spec.General.ServiceName, cr.Namespace)
}

func StsName(cr *eliatrav1.OpenSearchCluster, nodePool *eliatrav1.NodePool) string {
	return cr.Name + "-" + nodePool.Component
}

func DiscoveryServiceName(cr *eliatrav1.OpenSearchCluster) string {
	return fmt.Sprintf("%s-discovery", cr.Name)
}

func BootstrapPodName(cr *eliatrav1.OpenSearchCluster) string {
	return fmt.Sprintf("%s-bootstrap-0", cr.Name)
}

func STSInNodePools(sts appsv1.StatefulSet, nodepools []eliatrav1.NodePool) bool {
...
func AllMastersReady(ctx context.Context, k8sClient client.Client, cr *eliatrav1.OpenSearchCluster) bool {
...
func NewServiceMonitor(cr *eliatrav1.OpenSearchCluster) *monitoring.ServiceMonitor {
	labels := map[string]string{
		helpers.ClusterLabel: cr.Name,
	}
	selector := metav1.LabelSelector{
