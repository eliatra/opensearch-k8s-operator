...
func (r *ActionGroupReconciler) Reconcile() (retResult ctrl.Result, retErr error) {
	...
	defer func() {
		...
		err := r.client.UdateObjectStatus(r.instance, func(object client.Object) {
			instance := object.(*eliatrav1.OpensearchActionGroup)
			instance.Status.Reason = reason
			if retErr != nil {
				instance.Status.State = eliatrav1.OpensearchActionGroupError
			}
			if retResult.Requeue && retResult.RequeueAfter == 10*time.Second {
				instance.Status.State = eliatrav1.OpensearchActionGroupPending
			}
			if retErr == nil && retResult.RequeueAfter == 30*time.Second {
				instance.Status.State = eliatrav1.OpensearchActionGroupCreated
			}
			if reason == opensearchActionGroupExists {
				instance.Status.State = eliatrav1.OpensearchActionGroupIgnored
			}
		})
		if err != nil {
