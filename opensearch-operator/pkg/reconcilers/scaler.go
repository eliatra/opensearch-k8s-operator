...
func (r *ScalerReconciler) reconcileNodePool(nodePool *eliatrav1.NodePool) (bool, error) {
...
func (r *ScalerReconciler) decreaseOneNode(currentStatus eliatrav1.ComponentStatus, currentSts appsv1.StatefulSet, nodePoolGroupName string, smartDecrease bool) (bool, error) {
	lg := log.FromContext(r.ctx)
	*currentSts.Spec.Replicas--
	annotations := map[string]string{"cluster-name": r.instance.GetName()}
	lastReplicaNodeName := helpers.ReplicaHostName(currentSts, *currentSts.Spec.Replicas)
...
func (r *ScalerReconciler) excludeNode(currentStatus eliatrav1.ComponentStatus, currentSts appsv1.StatefulSet, nodePoolGroupName string) error {
	lg := log.FromContext(r.ctx)
	username, password, err := helpers.UsernameAndPassword(r.client, r.instance)
	annotations := map[string]string{"cluster-name": r.instance.GetName()}
	if err != nil {
...
func (r *ScalerReconciler) drainNode(currentStatus eliatrav1.ComponentStatus, currentSts appsv1.StatefulSet, nodePoolGroupName string) error {
	lg := log.FromContext(r.ctx)
	annotations := map[string]string{"cluster-name": r.instance.GetName()}
	lastReplicaNodeName := helpers.ReplicaHostName(currentSts, *currentSts.Spec.Replicas-1)
	username, password, err := helpers.UsernameAndPassword(r.client, r.instance)
