...
func (r *UserReconciler) Reconcile() (retResult ctrl.Result, retErr error) {
	...
	defer func() {
		...
		err := r.client.UdateObjectStatus(r.instance, func(object client.Object) {
			instance := object.(*eliatrav1.OpensearchUser)
			instance.Status.Reason = reason
			if retErr != nil {
				instance.Status.State = eliatrav1.OpensearchUserStateError
			}
			if retResult.Requeue && retResult.RequeueAfter == 10*time.Second {
				instance.Status.State = eliatrav1.OpensearchUserStatePending
			}
			if retErr == nil && retResult.RequeueAfter == 30*time.Second {
				instance.Status.State = eliatrav1.OpensearchUserStateCreated
			}
		})
		if err != nil {
