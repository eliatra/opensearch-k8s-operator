...
func newDashboardsReconciler(k8sClient *k8s.MockK8sClient, spec *eliatrav1.OpenSearchCluster) (ReconcilerContext, *DashboardsReconciler) {
		...
		It("should mount the secret", func() {
			...
			spec := eliatrav1.OpenSearchCluster{
				...
				Spec: eliatrav1.ClusterSpec{
					General: eliatrav1.GeneralConfig{ServiceName: clusterName},
					Dashboards: eliatrav1.DashboardsConfig{
						Enable: true,
						Tls: &eliatrav1.DashboardsTlsConfig{
							Enable:               true,
							Generate:             false,
							TlsCertificateConfig: eliatrav1.TlsCertificateConfig{Secret: corev1.LocalObjectReference{Name: secretName}},
						},
					},
				},
			}
